...
class TestSimpleDibrSoftMask:
    ...
    def selected_face_idx(self, height, width, face_vertices_image,
        ...
        _, face_idx = kaolin.render.mesh.rasterize(
    ...
    def test_C_dibr_soft_mask_forward(
        ...
        soft_mask, close_face_dist, close_face_idx, close_face_dist_type = \
            kaolin._C.render.mesh.dibr_soft_mask_forward_cuda(
                face_vertices_image,
                face_large_bboxes,
                selected_face_idx,
                sigmainv,
    ...
    def test_dibr_soft_mask_forward(self, face_vertices_image, selected_face_idx,
        ...
        soft_mask = kaolin.render.mesh.dibr_soft_mask(
            face_vertices_image,
            selected_face_idx,
            sigmainv,
            boxlen,
    ...
    def test_dibr_soft_mask_backward(self, face_vertices_image, selected_face_idx,
        ...
        loss = kaolin.metrics.render.mask_iou(soft_mask, shifted_mask)
        loss.backward()
...
class TestDibrSoftMask:
    @pytest.fixture(autouse=True)
    def mesh(self):
        mesh = kaolin.io.obj.import_mesh(os.path.join(MODEL_DIR, 'model.obj'),
    ...
    def camera_proj(self, dtype):
        return kaolin.render.camera.generate_perspective_projection(
    ...
    def vertices_camera(self, mesh, camera_pos, look_at, camera_up, dtype):
        ...
        camera_rot, camera_trans = kaolin.render.camera.generate_rotate_translate_matrices(
            camera_pos, look_at, camera_up)
        return kaolin.render.camera.rotate_translate_points(
            vertices, camera_rot, camera_trans)

    @pytest.fixture(autouse=True)
    def vertices_image(self, vertices_camera, camera_proj):
        return kaolin.render.camera.perspective_camera(
            vertices_camera, camera_proj)

    @pytest.fixture(autouse=True)
    def face_vertices_z(self, vertices_camera, faces):
        return kaolin.ops.mesh.index_vertices_by_faces(
...
class TestDibrRasterization:
    ...
    def face_normals_z(self, face_vertices_camera):
        return kaolin.ops.mesh.face_normals(
    ...
    def test_dibr_rasterization(self, height, width, face_vertices_z,
        ...
        gt_soft_mask = kaolin.render.mesh.dibr_soft_mask(
            face_vertices_image,
            gt_face_idx,
            sigmainv,
            boxlen,
        ...
        interpolated_features, soft_mask, face_idx = kaolin.render.mesh.dibr_rasterization(
            height, width,
            face_vertices_z,
            face_vertices_image,
            face_uvs,
